package com.example.robottime.voice;
...
public class RecognizerTask implements Runnable {
	...
	public RecognizerTask(String hmm, String lm, String log) {
		pocketsphinx
				.setLogfile(log);
		Config c = new Config();
		/*
		...
		Log.d(TAG, hmm);
		Log.d(TAG, "***************" + FileUtil.isFolderExists(hmm, true));

		Log.d(TAG, dicF);
		Log.d(TAG, "***************" + FileUtil.isFileExists(dicF));

		Log.d(TAG, lmF);
		Log.d(TAG, "***************" + FileUtil.isFileExists(lmF));

		if(!FileUtil.isFolderExists(hmm, true) ||
			!FileUtil.isFileExists(dicF) ||
			!FileUtil.isFileExists(lmF))
		{
			Log.e(TAG, "lm / dic file load failed ...");
			return;
		}

		c.setString("-hmm", hmm);
		c.setString("-dict", dicF);
		c.setString("-lm", lmF);
