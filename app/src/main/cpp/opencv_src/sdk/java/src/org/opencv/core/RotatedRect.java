...
public class RotatedRect {
    ...
    public RotatedRect() {
        this.center = new org.opencv.core.Point();
        this.size = new org.opencv.core.Size();
        this.angle = 0;
    }

    public RotatedRect(org.opencv.core.Point c, Size s, double a) {
        this.center = c.clone();
        this.size = s.clone();
        this.angle = a;
    }
    ...
    public void points(org.opencv.core.Point pt[])
    {
        double _angle = angle * Math.PI / 180.0;
        double b = (double) Math.cos(_angle) * 0.5f;
        double a = (double) Math.sin(_angle) * 0.5f;

        pt[0] = new org.opencv.core.Point(
                center.x - a * size.height - b * size.width,
                center.y + b * size.height - a * size.width);

        pt[1] = new org.opencv.core.Point(
                center.x + a * size.height - b * size.width,
                center.y - b * size.height - a * size.width);

        pt[2] = new org.opencv.core.Point(
                2 * center.x - pt[0].x,
                2 * center.y - pt[0].y);

        pt[3] = new org.opencv.core.Point(
                2 * center.x - pt[1].x,
                2 * center.y - pt[1].y);
    }
    ...
    {
        org.opencv.core.Point pt[] = new Point[4];
        points(pt);
        org.opencv.core.Rect r = new Rect((int) Math.floor(Math.min(Math.min(Math.min(pt[0].x, pt[1].x), pt[2].x), pt[3].x)),
